import{T as e,b as t,d as n,i as r,n as i,o as a,p as o,r as s,t as c,u as l,x as u}from"./index-nz68U_cs.js";import{s as d}from"./city-zoom-layout-1Sxt5jj9.js";function f(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function p(e){return e.replace(/[<>&\u2028\u2029]/g,e=>{switch(e){case`<`:return`\\u003c`;case`>`:return`\\u003e`;case`&`:return`\\u0026`;case`\u2028`:return`\\u2028`;case`\u2029`:return`\\u2029`;default:return e}})}function m(e,t=`#4f8c7c`){return/^#[0-9a-f]{6}$/i.test(e.trim())?e.trim():t}var h=1e5,g=85.05112878;function _([e,t]){let n=Math.max(-85.05112878,Math.min(g,t)),r=e*Math.PI/180,i=n*Math.PI/180,a=Math.log(Math.tan(Math.PI/4+i/2));return[r*h,-a*h]}function v(e){let t=o(e.document),n=e.document.transfers.flatMap(e=>{let n=t.stopCoordinates.get(e.fromStopId),r=t.stopCoordinates.get(e.toStopId);return!n||!r?[]:[{label:e.label,from:_(n),to:_(r)}]});return{days:t.days.map(e=>({id:e.day.id,path:e.path.map(_),hull:e.hull.map(_),labelPosition:_(e.labelPosition),stops:e.stops.map(({stop:e,coordinate:t})=>({id:e.id,point:_(t),navigationUrl:u(e)}))})),transfers:n,unplannedStops:(e.document.unplannedStops??[]).map(e=>({id:e.id,navigationUrl:u(e)}))}}function y(e){if(e.provider!==`amap`)return null;let t=e.url.trim();if(!t)return null;try{let n=new URL(t);if(n.protocol!==`https:`&&n.protocol!==`http:`)return null;n.protocol=`https:`;let r=e.title?.trim();return{provider:`amap`,url:n.toString(),...r?{title:r}:{}}}catch{return null}}function b(e,t){let n=new Set([...e.document.days.flatMap(e=>e.stops.map(e=>e.id)),...(e.document.unplannedStops??[]).map(e=>e.id)]),r=[];for(let e of n){let n=[],i=new Set;for(let r of t.get(e)??[]){let e=y(r);if(!(!e||i.has(e.url))&&(i.add(e.url),n.push(e),n.length===6))break}n.length&&r.push({stopId:e,photos:n})}return{resolvedPhotos:r}}function x(e){return new Map(e.resolvedPhotos.map(e=>[e.stopId,e.photos]))}var ee=String.raw`
(()=>{'use strict';
...
function getStopTimeLabel(stop){
  const schedule=stop.schedule;
  if(!schedule)return stop.time;
  if(!schedule.time)return'灵活安排';
  return schedule.kind==='fixed'?schedule.time+' · 预约':'约 '+schedule.time;
}

function getStopDurationLabel(stop){
  if(!stop.schedule)return stop.duration||'';
  const minutes=stop.schedule.durationMinutes;
...
function showPlace(stop,context,navigationUrl,marker){
  ...
  detailMeta.textContent=[context,stop.city,getStopTimeLabel(stop)].filter(Boolean).join(' · ');
  detailTitle.textContent=stop.name;
  detailShort.textContent=[stop.short,getStopDurationLabel(stop)].filter(Boolean).join(' · ');
  detailSummary.textContent=[stop.schedule&&stop.schedule.note,stop.summary].filter(Boolean).join('\n\n');
...
`,te=1440,ne=900,S=1100,C=620,re=85.05112878,ie=6371e3,w={originX:720,originY:690,centerX:S/2,centerY:C/2,xAxisX:.84,xAxisY:.12,yAxisX:-.36,yAxisY:.44,firstLayerHeight:28,layerGap:40};function T(e){return Number(e.toFixed(2))}function E(e,t){return{x:T(e),y:T(t)}}function D(e,t){return{x:e,y:t}}function O(e,t,n){return Math.max(t,Math.min(n,e))}function k([e,t]){let n=O(t,-85.05112878,re),r=e*Math.PI/180,i=n*Math.PI/180;return D(r,-Math.log(Math.tan(Math.PI/4+i/2)))}function ae(e,t){let n=(t[1]-e[1])*Math.PI/180,r=(t[0]-e[0])*Math.PI/180,i=e[1]*Math.PI/180,a=t[1]*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(i)*Math.cos(a)*Math.sin(r/2)**2;return 2*ie*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}function A(e){return e.normalize(`NFKC`).toLocaleLowerCase(`zh-CN`).replace(/[\s·•.,，。()（）【】[\]_-]/g,``)}function j(e,t){let n=2166136261;for(let e of t)n^=e.codePointAt(0)??0,n=Math.imul(n,16777619);return`${e}-${(n>>>0).toString(36)}`}function M(e,t){let n=e.stop.placeProvider?.placeId,r=t.stop.placeProvider?.placeId;if(n&&r&&n===r)return!0;let i=ae(e.coordinate,t.coordinate);return i<=12||i<=250&&A(e.stop.name)===A(t.stop.name)}function oe(e){return o(e).days.flatMap((e,t)=>e.stops.map(({stop:e,coordinate:n},r)=>({key:`${t}:${r}`,stop:e,coordinate:n,projected:k(n),dayIndex:t,stopIndex:r})))}function se(e){let t=e.map((e,t)=>t),n=e=>(t[e]!==e&&(t[e]=n(t[e])),t[e]),r=(e,r)=>{let i=n(e),a=n(r);i!==a&&(t[a]=i)};for(let t=0;t<e.length;t+=1)for(let n=t+1;n<e.length;n+=1)M(e[t],e[n])&&r(t,n);let i=new Map;return e.forEach((e,t)=>{let r=n(t),a=i.get(r);a?a.push(e):i.set(r,[e])}),[...i.values()].map((e,t)=>{let n=[e.reduce((e,t)=>e+t.coordinate[0],0)/e.length,e.reduce((e,t)=>e+t.coordinate[1],0)/e.length],r=D(e.reduce((e,t)=>e+t.projected.x,0)/e.length,e.reduce((e,t)=>e+t.projected.y,0)/e.length),i=d(e[0].stop.city);return{key:`place-${t}`,name:e[0].stop.name,cityId:j(`guide-city`,i),coordinate:n,projected:r,occurrences:e}})}function ce(e){let t=new Map((e.metro?.lines??[]).map(e=>[e.id,e])),n=new Map((e.metro?.stations??[]).map(e=>[e.id,e])),r=new Map(e.days.flatMap((e,t)=>e.stops.map(e=>[e.id,t]))),i=new Set,a=[];for(let n of e.metro?.journeys??[]){if(n.status!==`subway`)continue;let e=r.get(n.fromStopId);if(e!==void 0)for(let r of n.segments){let n=t.get(r.lineId);!n||r.path.length<2||(r.stationIds.forEach(e=>i.add(e)),a.push({id:r.id,lineId:n.id,lineName:n.name,color:n.color,cityId:j(`guide-city`,d(n.city)),dayIndex:e,stationIds:r.stationIds,projectedPath:r.path.map(k)}))}}return{stations:[...i].flatMap(e=>{let t=n.get(e);return t?[{id:t.id,name:t.name,cityId:j(`guide-city`,d(t.city)),lineIds:t.lineIds,projected:k(t.coordinate)}]:[]}),segments:a}}function N(e){if(e.length===0)return{minX:0,maxX:1,minY:0,maxY:1,width:1,height:1,centerX:.5,centerY:.5};let t=Math.min(...e.map(e=>e.x)),n=Math.max(...e.map(e=>e.x)),r=Math.min(...e.map(e=>e.y)),i=Math.max(...e.map(e=>e.y));return{minX:t,maxX:n,minY:r,maxY:i,width:Math.max(n-t,1e-6),height:Math.max(i-r,1e-6),centerX:(t+n)/2,centerY:(r+i)/2}}function P(e,t){let n=F(e,t);return e.map(n)}function F(e,t){let n=N(e),r=Math.min(t.width/n.width,t.height/n.height);return e=>E(t.center.x+(e.x-n.centerX)*r,t.center.y+(e.y-n.centerY)*r)}function le(e){let t=P(e.map(e=>e.projected),{center:E(S/2,C/2),width:920,height:470});return Object.fromEntries(e.map((e,n)=>[e.key,t[n]]))}function I(e,t){let n=F(e.map(e=>e.projected),{center:E(S/2,C/2),width:920,height:470});return{stations:Object.fromEntries(t.stations.map(e=>[e.id,n(e.projected)])),segments:Object.fromEntries(t.segments.map(e=>[e.id,e.projectedPath.map(n)]))}}function L(e){let t=new Map;for(let n of e){let e=t.get(n.cityId);e?e.push(n):t.set(n.cityId,[n])}let n=[...t.entries()].map(([e,t])=>({cityId:e,center:D(t.reduce((e,t)=>e+t.projected.x,0)/t.length,t.reduce((e,t)=>e+t.projected.y,0)/t.length)}));if(n.length===0)return[];if(n.length===1)return[{cityId:n[0].cityId,center:E(550,310),width:860,height:470}];if(n.length===2){let[e,t]=n,r=Math.sign(t.center.x-e.center.x)||1,i=Math.sign(t.center.y-e.center.y)||-1,a=r*270,o=i*130;return[{cityId:e.cityId,center:E(550-a,310-o),width:440,height:320},{cityId:t.cityId,center:E(550+a,310+o),width:440,height:320}]}let r=P(n.map(e=>e.center),{center:E(550,305),width:700,height:360}),i=n.length<=4?340:280,a=n.length<=4?250:210;return n.map((e,t)=>({cityId:e.cityId,center:r[t],width:i,height:a}))}function R(e,t){let n=new Set;return e.days.forEach((e,r)=>{let i=e.stops.map((e,n)=>e.includeInRoute===!1?null:t.get(`${r}:${n}`)).filter(e=>!!e);for(let e=1;e<i.length;e+=1)i[e-1]!==i[e]&&n.add([i[e-1],i[e]].sort().join(`|`))}),[...n].map(e=>{let[t,n]=e.split(`|`);return[t,n]})}function z(e,t,n){let r=P(e.map(e=>e.projected),{center:t.center,width:Math.max(120,t.width-96),height:Math.max(100,t.height-88)}),i=new Map;e.forEach((e,t)=>i.set(e.key,E(Math.round(r[t].x/12)*12,Math.round(r[t].y/12)*12)));let a=new Map(e.map((e,t)=>[e.key,r[t]])),o=new Set(e.map(e=>e.key)),s=n.filter(([e,t])=>o.has(e)&&o.has(t)),c=e.length>13?52:e.length>8?60:68;for(let n=0;n<48;n+=1){for(let t of e){let e=i.get(t.key),n=a.get(t.key);e.x+=(n.x-e.x)*.08,e.y+=(n.y-e.y)*.08}for(let[e,t]of s){let n=i.get(e),r=i.get(t),a=r.x-n.x,o=r.y-n.y,s=Math.hypot(a,o)||1,c=(s-82)*.045,l=a/s,u=o/s;n.x+=l*c,n.y+=u*c,r.x-=l*c,r.y-=u*c}for(let t=0;t<e.length;t+=1)for(let n=t+1;n<e.length;n+=1){let r=i.get(e[t].key),a=i.get(e[n].key),o=a.x-r.x,s=a.y-r.y,l=Math.hypot(o,s)||.001;if(l>=c)continue;let u=(c-l)*.22,d=o/l,f=s/l;r.x-=d*u,r.y-=f*u,a.x+=d*u,a.y+=f*u}let n=t.width/2-30,r=t.height/2-28;for(let a of e){let e=i.get(a.key);e.x=O(e.x,t.center.x-n,t.center.x+n),e.y=O(e.y,t.center.y-r,t.center.y+r)}}return Object.fromEntries(e.map(e=>{let t=i.get(e.key);return[e.key,E(Math.round(t.x/4)*4,Math.round(t.y/4)*4)]}))}function B(e,t,n){let r=R(e,n),i=new Map(L(t).map(e=>[e.cityId,e])),a=new Map;for(let e of t){let t=a.get(e.cityId);t?t.push(e):a.set(e.cityId,[e])}return Object.assign({},...[...a.entries()].map(([e,t])=>{let n=i.get(e);return n?z(t,n,r):{}}))}function V(e,t){let n=new Map(L(e).map(e=>[e.cityId,e])),r={},i={},a=new Set([...t.stations.map(e=>e.cityId),...t.segments.map(e=>e.cityId)]);for(let o of a){let a=n.get(o);if(!a)continue;let s=e.filter(e=>e.cityId===o),c=t.stations.filter(e=>e.cityId===o),l=t.segments.filter(e=>e.cityId===o),u=F([...s.map(e=>e.projected),...c.map(e=>e.projected),...l.flatMap(e=>e.projectedPath)],{center:a.center,width:Math.max(120,a.width-70),height:Math.max(100,a.height-64)});for(let e of c)r[e.id]=u(e.projected);for(let e of l)i[e.id]=e.projectedPath.map(u)}return{stations:r,segments:i}}function H(e,t,n,r){let i=N(e),a=Math.max(n/2,i.width/2+t),o=Math.max(r/2,i.height/2+t),s=Math.min(30,a*.16,o*.2),{centerX:c,centerY:l}=i;return[E(c-a+s,l-o),E(c+a-s,l-o),E(c+a,l-o+s),E(c+a,l+o-s),E(c+a-s,l+o),E(c-a+s,l+o),E(c-a,l+o-s),E(c-a,l-o+s)]}function U(e,t){let n=N(e),r=Math.max(120,n.width/2+56),i=Math.max(92,n.height/2+48);return Array.from({length:14},(e,a)=>{let o=Math.PI*2*a/14,s=1+Math.sin((a+1)*2.17+t*.73)*.055;return E(n.centerX+Math.cos(o)*r*s,n.centerY+Math.sin(o)*i*s)})}function W(e,t,n,r,i,a,o){return{places:e,metroStations:r,metroSegments:i,cities:Object.fromEntries(t.map((t,n)=>{let s=a.filter(e=>e.cityId===t.id).map(e=>e.id),c=o.filter(e=>e.cityId===t.id).map(e=>e.id),l=[...t.placeKeys.map(t=>e[t]).filter(Boolean),...s.map(e=>r[e]).filter(Boolean),...c.flatMap(e=>i[e]??[])],u=N(l);return[t.id,{outline:U(l,n+1),label:E(u.centerX,u.centerY+Math.max(86,u.height/2+38))}]})),days:Object.fromEntries(n.map(t=>{let n=H((t.hullPlaceKeys.length?t.hullPlaceKeys:t.routePlaceKeys).map(t=>e[t]).filter(Boolean),46,230,150),r=N(n);return[t.id,{plane:n,label:E(O(r.minX-140,16,930),48)}]}))}}function G(e,t,n=0){let r=e.projection,i=t.x-r.centerX,a=t.y-r.centerY;return E(r.originX+i*r.xAxisX+a*r.yAxisX,r.originY+i*r.xAxisY+a*r.yAxisY-n)}function K(e,t){return e.projection.firstLayerHeight+t*e.projection.layerGap}function ue(e){let t=oe(e),n=se(t),r=ce(e),i=new Map;n.forEach(e=>e.occurrences.forEach(t=>i.set(t.key,e)));let a=new Map([...i.entries()].map(([e,t])=>[e,t.key])),o=new Map,s=new Map;for(let e of n){let t=d(e.occurrences[0].stop.city);o.set(e.cityId,t);let n=s.get(e.cityId)??new Set;n.add(e.key),s.set(e.cityId,n)}for(let t of r.stations){s.has(t.cityId)||s.set(t.cityId,new Set);let n=e.metro?.stations.find(e=>e.id===t.id);n&&o.set(t.cityId,d(n.city))}let c=[...s.entries()].map(([e,t])=>({id:e,name:o.get(e)??`待定城市`,placeKeys:[...t]})),l=t.map(e=>{let t=i.get(e.key);return{key:e.key,id:e.stop.id,placeKey:t.key,cityId:t.cityId,dayIndex:e.dayIndex,stopIndex:e.stopIndex}}),u=new Map(l.map(e=>[e.id,e])),f=e.days.map((e,t)=>({id:e.id,index:e.index,city:e.city,title:e.title,routeTitle:e.routeTitle,durationLabel:e.durationLabel,color:e.color,routePlaceKeys:e.stops.flatMap((e,n)=>e.includeInRoute===!1?[]:[a.get(`${t}:${n}`)]),hullPlaceKeys:e.stops.flatMap((e,n)=>e.includeInHull===!1?[]:[a.get(`${t}:${n}`)]),stopKeys:e.stops.map((e,n)=>`${t}:${n}`)})),p=n.map(e=>({key:e.key,name:e.name,cityId:e.cityId,stopKeys:e.occurrences.map(e=>e.key)})),m=n.flatMap(e=>{let t=[...new Set(e.occurrences.map(e=>e.dayIndex))].sort((e,t)=>e-t);return t.length<2?[]:[{id:j(`shared-place`,e.key),name:e.name,placeKey:e.key,stopKeys:e.occurrences.map(e=>e.key),dayIndexes:t}]}),h=e.transfers.flatMap(e=>{let t=u.get(e.fromStopId),n=u.get(e.toStopId);return!t||!n?[]:[{id:e.id,label:e.label,mode:e.mode,fromStopKey:t.key,toStopKey:n.key}]}),g=r.stations.map(e=>({id:e.id,name:e.name,cityId:e.cityId,lineIds:e.lineIds})),_=r.segments.map(e=>({id:e.id,lineId:e.lineId,lineName:e.lineName,color:e.color,cityId:e.cityId,dayIndex:e.dayIndex,stationIds:e.stationIds})),v=le(n),y=I(n,r),b=B(e,n,a),x=V(n,r);return{viewBox:{width:te,height:ne},plane:{width:S,height:C},projection:w,places:p,stops:l,cities:c,days:f,sharedPlaces:m,transfers:h,metroStations:g,metroSegments:_,modes:{relative:W(b,c,f,x.stations,x.segments,g,_),geographic:W(v,c,f,y.stations,y.segments,g,_)}}}function q(e){return Number(e.toFixed(2)).toString()}function J(e){return`${q(e.x)},${q(e.y)}`}function Y(e){return e.map(J).join(` `)}function X(e,t,n=0){return t.map(t=>G(e,t,n))}function Z(e){return e.length<2?``:n(l(e.map(e=>[e.x,e.y])))}function de(e){return Math.max(52,Math.min(138,22+[...e].length*14))}function fe(r,i,a){let o=i.modes.relative,s=new Map(i.stops.map(e=>[e.key,e])),c=i.stops.flatMap(e=>{let n=r.document.days[e.dayIndex]?.stops[e.stopIndex];return!n||!t(n,a.get(n.id)??[])?[]:[`<clipPath id="guide-photo-${e.dayIndex}-${e.stopIndex}" clipPathUnits="userSpaceOnUse"><circle cx="0" cy="0" r="17" /></clipPath>`]}).join(``),u=i.cities.map(e=>{let t=o.cities[e.id],n=X(i,t.outline),r=G(i,t.label);return`<g class="map-city" data-city-id="${f(e.id)}">
      <polygon class="map-city__surface" data-city-outline="${f(e.id)}" points="${Y(n)}" />
      <polygon class="map-city__contour" data-city-outline="${f(e.id)}" points="${Y(n)}" />
      <g class="map-city__label" data-city-label="${f(e.id)}" transform="translate(${q(r.x)} ${q(r.y)})">
        <text class="map-city__name" text-anchor="middle">${f(e.name)}</text>
      </g>
    </g>`}).join(``),d=i.metroSegments.map(e=>{let t=o.metroSegments[e.id]??[];if(t.length<2)return``;let n=Z(X(i,t)),r=m(e.color);return`<g class="atlas-metro-segment" data-metro-day-index="${e.dayIndex}" style="--metro-color:${r}">
      <path class="atlas-metro__underlay" data-metro-segment="${f(e.id)}" d="${n}" />
      <path class="atlas-metro__route" data-metro-segment="${f(e.id)}" d="${n}" />
      <path class="atlas-metro__stitch" data-metro-segment="${f(e.id)}" d="${n}" />
    </g>`}).join(``),p=new Map(i.metroSegments.map(e=>[e.lineId,e])),h=i.metroStations.map(e=>{let t=o.metroStations[e.id];if(!t)return``;let n=G(i,t),r=m(e.lineIds.map(e=>p.get(e)).find(Boolean)?.color??`#4f8c7c`);return`<g class="atlas-metro-station${e.lineIds.length>1?` is-interchange`:``}" data-metro-station="${f(e.id)}" transform="translate(${q(n.x)} ${q(n.y)})" style="--metro-color:${r}">
      <circle r="${e.lineIds.length>1?5.5:4.5}" />
      <title>${f(e.name)}</title>
    </g>`}).join(``),g=new Set,_=i.metroSegments.flatMap(e=>{if(g.has(e.lineId))return[];let t=o.metroSegments[e.id]??[],n=t[Math.floor(t.length/2)];if(!n)return[];g.add(e.lineId);let r=G(i,n);return[`<text class="atlas-metro-label" data-metro-label="${f(e.lineId)}" data-metro-label-segment="${f(e.id)}" transform="translate(${q(r.x)} ${q(r.y-12)})" text-anchor="middle" style="--metro-color:${m(e.color)}">${f(e.lineName)}</text>`]}).join(``),v=i.sharedPlaces.map(e=>{let t=o.places[e.placeKey],n=K(i,e.dayIndexes[0]),r=K(i,e.dayIndexes.at(-1)??e.dayIndexes[0]),a=G(i,t,n),s=G(i,t,r);return`<g class="shared-place" data-shared-place="${f(e.id)}" aria-label="${f(e.name)}跨日连接">
      <path class="shared-place__halo" data-shared-stem="${f(e.id)}" d="M ${J(a)} L ${J(s)}" />
      <path class="shared-place__stem" data-shared-stem="${f(e.id)}" d="M ${J(a)} L ${J(s)}" />
    </g>`}).join(``),y=i.transfers.map(e=>{let t=s.get(e.fromStopKey),r=s.get(e.toStopKey);if(!t||!r)return``;let a=G(i,o.places[t.placeKey],K(i,t.dayIndex)),c=G(i,o.places[r.placeKey],K(i,r.dayIndex)),u=l([[a.x,a.y],[c.x,c.y]]);return`<g class="atlas-transfer" data-transfer-id="${f(e.id)}">
      <path class="atlas-transfer__underlay" data-transfer-path="${f(e.id)}" d="${n(u)}" />
      <path class="atlas-transfer__route" data-transfer-path="${f(e.id)}" d="${n(u)}" />
    </g>`}).join(``),b=i.days.map((e,n)=>{let c=r.document.days[n],l=o.days[e.id],u=K(i,n),d=X(i,l.plane,u),p=Z(e.routePlaceKeys.map(e=>G(i,o.places[e],u))),h=m(c.color),g=e.stopKeys.map((r,l)=>{let d=s.get(r);if(!d)return``;let p=c.stops[d.stopIndex],m=G(i,o.places[d.placeKey],u),h=t(p,a.get(p.id)??[]),g=de(p.name),_=h?`<circle class="atlas-stop__fallback" r="17" /><path class="atlas-stop__fallback-icon" d="M0-7c-4 0-7.1 3.1-7.1 7.2C-7.1 5 0 10.4 0 10.4S7.1 5 7.1.2C7.1-3.9 4-7 0-7Zm0 4a3.1 3.1 0 1 1 0 6.2A3.1 3.1 0 0 1 0-3Z" /><image class="atlas-stop__image" href="${f(h.src)}" x="-17" y="-17" width="34" height="34" clip-path="url(#guide-photo-${n}-${d.stopIndex})" preserveAspectRatio="xMidYMid slice" aria-hidden="true" />`:`<circle class="atlas-stop__pin atlas-stop__fallback" r="17" /><path class="atlas-stop__fallback-icon" d="M0-7c-4 0-7.1 3.1-7.1 7.2C-7.1 5 0 10.4 0 10.4S7.1 5 7.1.2C7.1-3.9 4-7 0-7Zm0 4a3.1 3.1 0 1 1 0 6.2A3.1 3.1 0 0 1 0-3Z" />`;return`<g class="atlas-stop${l===0||l===e.stopKeys.length-1?` is-route-anchor`:``}" data-stop-key="${d.key}" data-place-key="${d.placeKey}" data-photo-source="${h?.source??`fallback`}" transform="translate(${q(m.x)} ${q(m.y)})" role="button" tabindex="0" aria-label="第 ${c.index} 天第 ${d.stopIndex+1} 站，${f(p.name)}">
        ...
        <text class="atlas-stop__number" x="-15" y="-14" text-anchor="middle" dominant-baseline="central">${d.stopIndex+1}</text>
        <rect class="atlas-stop__name-bg" x="${q(-g/2)}" y="24" width="${g}" height="23" rx="7" />
        <text class="atlas-stop__name" x="0" y="39" text-anchor="middle">${f(p.name)}</text>
      </g>`}).join(``);return`<g class="atlas-day" data-day-index="${n}" data-day-id="${f(e.id)}" style="--day-color:${h}">
      <polygon class="atlas-day__hull" data-day-plane="${f(e.id)}" points="${Y(d)}" />
      ${p?`<path class="atlas-day__route-underlay" data-day-route="${f(e.id)}" d="${p}" /><path class="atlas-day__route" data-day-route="${f(e.id)}" d="${p}" stroke-dasharray="9 8" stroke-linecap="round" stroke-linejoin="round" />`:``}
      ${g}
    </g>`}).join(``),x=i.days.map((t,n)=>{let a=r.document.days[n],s=o.days[t.id],c=G(i,s.label,K(i,n));return`<g class="atlas-day-label" data-day-label="${f(t.id)}" data-day-trigger="${n}" transform="translate(${q(c.x)} ${q(c.y)})" role="button" tabindex="0" aria-label="聚焦第 ${a.index} 天，${f(a.city)}" style="--day-color:${m(a.color)}">
      <rect x="-8" y="-13" width="118" height="32" rx="8" />
      <text class="atlas-day-label__day" x="4" y="7">${e(a.index)}</text>
      <text class="atlas-day-label__name" x="52" y="7">${f(a.city)}</text>
    </g>`}).join(``);return`<svg id="atlas" class="atlas" viewBox="0 0 ${i.viewBox.width} ${i.viewBox.height}" role="group" aria-labelledby="atlas-title atlas-description" preserveAspectRatio="xMidYMid meet" data-stop-count="${i.stops.length}">
    <title id="atlas-title">${f(r.document.title)}旅行路线</title>
    <desc id="atlas-description">按天叠放的旅行路线。可切换示意布局和实景布局，点击日期标签或地点查看重点。</desc>
    <defs>${c}</defs>
    <g id="scene-camera" class="scene-camera">
      <g class="map-plane" aria-label="城市底图">${u}</g>
      <g class="metro-network-layer" aria-label="相关地铁线路">${d}${h}${_}</g>
      <g class="shared-place-layer" aria-label="跨日共享地点">${v}</g>
      <g class="transfer-layer" aria-label="跨日交通">${y}</g>
      <g class="day-layer-stack" aria-label="每日行程平面">${b}</g>
      <g class="day-label-stack" aria-label="每日行程标签">${x}</g>
    </g>
  </svg>`}var pe=String.raw`
:root {
  color-scheme: light;
  --guide-canvas: #e6f3ed;
...
`,me=`travel-map-studio/standalone-guide`,Q=new Map;function $(e,t=Q){let n=r(e),a=v(n),o=b(n,t),c=x(o),l=ue(n.document),u=p(s(n).trim()),d=p(JSON.stringify(a)),m=p(JSON.stringify(o)),h=p(JSON.stringify(l)),g=fe(n,l,c),_=f(n.document.title),y=i();return`<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  ...
  <title>${_} · Travel Map Studio</title>
  <style>${pe}</style>
</head>
<body>
  <main id="guide" class="guide" data-guide-format="${me}" data-layout-mode="relative">
    <section id="atlas-stage" class="atlas-stage" aria-label="旅行路线地图">
      ${g}
      ${n.document.days.length===0?`<p class="empty-message">这个项目还没有行程。</p>`:``}
    </section>
  ...
  <script id="${y}" type="application/json">${u}<\/script>
  <script id="tms-geometry" type="application/json">${d}<\/script>
  <script id="tms-display-media" type="application/json">${m}<\/script>
  <script id="tms-scene" type="application/json">${h}<\/script>
  <script>${ee}<\/script>
</body>
</html>`}function he(e,t=Q){let n=$(e,t),r=new Blob([n],{type:`text/html;charset=utf-8`});c(r,`${a(e.document.title,`travel-map-guide`)}.guide.html`)}export{$ as createStandaloneGuideHtml,he as downloadStandaloneGuide,p as escapeJsonForHtml};